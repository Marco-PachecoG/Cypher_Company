    ...
    elControls.forEach((elControl) => {
      elControl.addEventListener("click", (e) => {
        ...
        if (elTarget.classList.contains("control--next")) {
          ...
          this.showCells();
        } else if (!(moment().format("MM") == this.currentMonth.format("MM"))) {
  ...
  showCells() {
    ...
    db.collection("messages").onSnapshot((snapshots) => {
      ...
      snapshots.forEach((doc) => {
        ...
        if (doc.data().month == this.currentMonth.format("MM")) {
          daysUsed.push(doc.data().day);
          console.log(daysUsed);
        }
      });
      this.cells = this.generateDates(this.currentMonth);
      //console.log(this.generateDates(this.currentMonth));
      if (this.cells === null) {
      ...
      let templateCells = "";
      let disabledClass = "";
      //Aquí se puede hacer la magia de desabilitar las fechas seleccionadas
      for (let i = 0; i < this.cells.length; i++) {
        disabledClass = "";
        let used = "Día disponible";
        if (!this.cells[i].isInCurrentMonth) {
          disabledClass = "grid__cell--disabled";
          used = "";
        } else if (daysUsed.indexOf(this.cells[i].date.date()) != -1) {
            disabledClass = "grid__cell--disabled grid__cell--disabled--used";
            used = "Día ocupado";
        }
        // <span class="grid__cell grid__cell--gd grid__cell--selected">1</span>
        templateCells += `
                <span class="grid__cell grid__cell--gd ${disabledClass}" data-cell-id="${i}" title="${used}">
                    ${this.cells[i].date.date()}
                </span>
            `;
        //console.log("=>: " + this.cells[i].date.date());
      }
      console.log("current => ", this.currentMonth.format("MM"));
  ...
  generateDates(monthToShow = moment()) {
    ...
    do {
      cells.push({
        ...
        isInCurrentMonth: dateStart.month() === monthToShow.month(), //para deshabilitar con un false || isUsed
      });
  ...
  addEventListenerToCells() {
    ...
    elCells.forEach((elCell) => {
      elCell.addEventListener("click", (e) => {
        ...
        this.selectedDate = this.cells[parseInt(elTarget.dataset.cellId)].date; //Obtiene la fecha seleccionada
